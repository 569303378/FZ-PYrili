...
- (UICollectionViewCell *)collectionView:(UICollectionView *)collectionView cellForItemAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if (indexPath.section == 0) {
        ...
        [cell.dateLabel setTextColor:[UIColor colorWithHexString:@"#15cc9c"]];//星期-颜色
    } else {
        ...
        if (i < firstWeekday) {
            ...
            [cell.dateLabel setTextColor:[UIColor colorWithHexString:@"#cbcbcb"]];//可选中 日期颜色

            //this month
            if ([_today isEqualToDate:_date]) {
                if (day == [self day:_date]) {
                    [cell.dateLabel setTextColor:[UIColor colorWithHexString:@"#4898eb"]];//今天 日期颜色
                } else if (day > [self day:_date]) {
                    [cell.dateLabel setTextColor:[UIColor colorWithHexString:@"#6f6f6f"]];//本月 可选颜色
                    //改变选中字体颜色
                    cell.dateLabel.highlightedTextColor = [UIColor cyanColor];                }
            } else if ([_today compare:_date] == NSOrderedAscending) {
                [cell.dateLabel setTextColor:[UIColor colorWithHexString:@"#6f6f6f"]];//次月 可选颜色
                //改变选中字体颜色
                cell.dateLabel.highlightedTextColor = [UIColor cyanColor];
            }
        }
    ...
    if (indexPath.section == 1) {
        ...
        if (i >= firstWeekday && i <= firstWeekday + daysInThisMonth - 1) {
            ...
            if ([_today isEqualToDate:_date]) {
                ...
                if (day >= [self day:_date]) {//判断今天-今后本月日期可选
                //    NSLog(@"%ld", (long)day);//day点击日期
                    return YES;
                }
            } else if ([_today compare:_date] == NSOrderedAscending) {//NSOrderedAscending 下月  NSOrderedDescending 上月
               // NSLog(@"%@, %@", _today, _date);//_today今天  _date上个月今天
                return YES;
            }
        }
    }
...
+ (instancetype)showOnView:(UIView *)view
{
    ...
    calendarPicker.mask.backgroundColor = [UIColor orangeColor];//背景颜色
